...
const { pollNvrDevice } = require('./isapiClient');
...
// Password decryption helper (for NVR login credentials)
...
const authenticateAgentToken = async (req, res, next) => {
  ...
  try {
    ...
    // Match raw token with stored hashed value or exact match for simplicity
...
router.post('/login', async (req, res) => {
  ...
  // Simple static config credentials check per PRD spec
...
router.post('/nvr', authenticateToken, async (req, res) => {
  ...
  try {
    // Validation: Duplicate IP + port check
...
router.delete('/nvr/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Prisma relation onDelete: Cascade handles Channels and HDDs deletion
...
router.post('/nvr/:id/poll', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (nvr.type === 'pcnvr') {
      // PCNVR is push model - manual refresh just updates timestamp or status of the agent
    ...
    // Hardware NVR: Execute active poll
