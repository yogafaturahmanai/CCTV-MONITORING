  ...
  try {
    ...
    for (const nvr of nvrs) {
      if (nvr.type === 'hardware_nvr') {
        // Hardware NVR uses pull model (poll via ISAPI Client)
        const result = await pollNvrDevice(nvr, decryptPassword(nvr.password_encrypted));
        
        // Update channels if status is online
        ...
        // PCNVR uses push model. Verify heartbeat status (PRD Section 16.1)
        if (nvr.last_heartbeat_at) {
          const lastHeartbeat = new Date(nvr.last_heartbeat_at);
          const diffSeconds = (new Date() - lastHeartbeat) / 1000;
          
          if (diffSeconds > 120) {
            // Heartbeat exceeded 2x interval (120s) -> mark agent stale
            // We do not overwrite channel details, just log the warning
...
app.listen(PORT, async () => {
  ...
  // Run periodic polling checker (every 60s)
